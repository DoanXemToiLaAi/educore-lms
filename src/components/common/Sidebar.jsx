...
export default function Sidebar({
  ...
  return (
    <>
      ...
      <div className={`fixed left-0 top-0 h-full bg-white dark:bg-gray-900 shadow-lg z-30 transition-all duration-300 border-r border-gray-200 dark:border-gray-700
            ...
            <li key={item.label}>
              <button
                ...
                className={`flex items-center gap-3 px-3 py-3 rounded-lg text-gray-700 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-200 group w-full text-left ${
                  activeMenu === idx ? "bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border-r-2 border-blue-600" : ""
                }`}
                onClick={(e) => {
                  e.preventDefault();
                  onMenuClick && onMenuClick(idx);
                }}>
                <i className={`bx ${item.icon} text-xl flex-shrink-0 ${
                  activeMenu === idx ? "text-blue-600 dark:text-blue-400" : ""
                }`}></i>
                {!isCollapsed && (
